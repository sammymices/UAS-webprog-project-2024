...
<body>
    <section id="hero">
    <!-- Navbar -->
    <nav class="navbar navbar-expand-lg navbar-light" id="navbar" style="background-image: inherit; background-color: inherit;">
        <div class="container">
    ...
    <div class="container mx-auto p-5">
        <div class="relative">
            @foreach ($programs as $index => $program)
                <div class="program-item" id="program-{{ $index }}" style="display: {{ $index === 0 ? 'block' : 'none' }}">
                    <h4 class="text-2xl font-bold">{{ $program->title }}</h4>
                    <img src="{{ asset($program->image) }}" alt="{{ $program->title }}" class="my-4">                    <p class="text-gray-700">{{ $program->description }}</p>
                </div>
            @endforeach

            <button id="prev" class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-green-500 text-white p-2 rounded">Kembali</button>
            <button id="next" class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-green-500 text-white p-2 rounded">Selanjutnya</button>
        </div>
    </div>
</div>
